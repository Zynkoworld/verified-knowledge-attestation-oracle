#!/usr/bin/env python3
"""verified-knowledge-attestation-oracle -- a deterministic attestation-integrity decider for the `verified-knowledge` knowledge domain.

An **oracle** *deterministically decides* the truth of a case. This one decides, for a `verified-knowledge` knowledge
capsule, whether it is a well-formed, provenance-carrying attestation. VALID iff: domain=='verified-knowledge', a non-empty
...
def decide(c):
    if c.get("domain") != "verified-knowledge": return "INVALID"
...
def _run_probes(path):
    ...
    print("verified-knowledge attestation oracle: probes=%d | recall=%.3f | false_positives=%d | verdict=%s"%(len(probes),recall,fp,"PASS" if ok else "FAIL"))
